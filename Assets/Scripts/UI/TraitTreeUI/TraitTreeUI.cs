...
public class TraitTreeUI : MonoBehaviour
{
    ...
    private void CreateNodeWidgets()
    {
        ...
        foreach (var node in _treeData.nodes)
        {
            ...
            var nodeUI = go.AddComponent<TraitNodeUI>();
    ...
    public void UpdateAllNodeStates()
    {
        ...
        HashSet<string> unlockedIDs = _traitTreeManager.GetUnlockedNodeIDs();
        ...
        foreach (var kvp in _nodeUILookup)
        {
            ...
            else if (unlockedIDs.Contains(nodeData.nodeID))
            {
                nodeUI.UpdateVisualState(TraitNodeState.Unlocked);
            }
            else if (IsNodeAvailable(nodeData, unlockedIDs))
            {
                bool canAfford = _traitTreeManager.CanAffordNode(nodeData);
                nodeUI.UpdateVisualState(canAfford ? TraitNodeState.Available : TraitNodeState.CannotAfford);
            }
            else
            {
                nodeUI.UpdateVisualState(TraitNodeState.Locked);
            }
        }

        foreach (var connUI in _connectionUIs)
            if (connUI != null) connUI.UpdateState(unlockedIDs);
    ...
    private bool IsNodeAvailable(TraitNode nodeData, HashSet<string> craftedIDs)
    {
        ...
        foreach (var conn in _treeData.connections)
        {
            ...
            if (conn.fromNodeIDs != null && conn.fromNodeIDs.Length > 0)
            {
                ...
                foreach (string fromID in conn.fromNodeIDs)
                {
                    ...
                    hasValid = true;
                    if (craftedIDs.Contains(fromID)) { anyMet = true; break; }
                }
    ...
    public void OnNodeClicked(TraitNodeUI nodeUI)
    {
        ...
        if (_traitTreeManager != null && !_traitTreeManager.CanAffordNode(node))
        {
            Debug.Log($"[TraitTreeUI] Node '{node.nodeID}' costs {node.goldCost} gold — not affordable.");
            return;
        }
    ...
    {
        if (traitData == null || TraitTooltip.Instance == null) return;

        string description = traitData.description ?? string.Empty;

        TraitNode node = nodeUI != null ? nodeUI.NodeData : null;
        if (node != null && node.goldCost > 0 && !_traitTreeManager.GetUnlockedNodeIDs().Contains(node.nodeID))
        {
            string colour = _traitTreeManager.CanAffordNode(node) ? "#00ff00" : "#ff4444";
            description += $"\n\n<b>Cost:</b> <color={colour}>{node.goldCost} gold</color>";
        }

        TraitTooltip.Instance.ShowTooltip(traitData.displayName, description);
    }
